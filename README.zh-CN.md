...
- **Obsidian 原生双链**：`[[笔记]]` 显示为链接文本，支持悬停预览与 `Ctrl+点击`（Windows/Linux）或 `Command+点击`（macOS）跳转；「添加链接」联想笔记与可链接附件。新增链接一律写为 `[[双链]]`（不跟随 Obsidian 的链接格式设置），以保证文档链接使用专属图标。**节点内只显示别名**（无别名时为笔记名）；纯双链节点里编辑节点文本即等于改别名，保存后回写为 `[[笔记|新别名]]`。
- **图片**：仓库内图片联想、统一尺寸、`![[路径]]` 往返；拖动右下角手柄即可调整节点图片大小，尺寸以 Obsidian 官方嵌入语法写回笔记（`![[图.png|300]]` 仅宽等比、`|300x150` 指定宽高、`![alt|300](url)` 外链图）；清空文本后节点被图片独占——该节点已无文本，故节点搜索不会命中它。
