...
// ---------------------------------------------------------------------------
// `---` 的两种身份：空行之后 = 结构分隔信号（丢弃，既定行为）；
// 紧跟非空行之后 = CommonMark 的 setext H2 下划线（必须保留，否则标题降级为段落）
// ---------------------------------------------------------------------------
describe('分隔线与 setext H2 下划线', () => {
	it('setext H2 下划线（标题行后的 `---`）逐字回写，不再降级为段落', () => {
		const md = '标题\n---';
		expect(roundTrip(md).out1, '未编辑：逐字回写').toBe(md);
		expect(roundTrip(md).out2, '不动点').toBe(md);
	});

	it('setext H2 下划线：5 个短横线与 ≤3 空格缩进同样保真', () => {
		expect(roundTrip('标题\n-----').out1).toBe('标题\n-----');
		expect(roundTrip('标题\n   ---').out1).toBe('标题\n   ---');
	});

	it('setext 标题 + 正文：整体逐字回写（下划线行不被吞）', () => {
		const md = '标题\n---\n正文';
		expect(roundTrip(md).out1).toBe(md);
	});

	it('对照：`=====`（setext H1）一直保留，两条 setext 形态现已对齐', () => {
		expect(roundTrip('标题\n=====').out1).toBe('标题\n=====');
	});

	it('对照：空行之后的分隔线仍按既定行为丢弃（结构分隔信号）', () => {
		expect(roundTrip('a\n\n---\n\nb').out1).toBe('a\nb');
	});
	it('对照：围栏内的 `---` 是代码内容，原样保留', () => {
		expect(roundTrip('```\n---\n```').out1).toBe('```\n---\n```');
	});

	it('列表/标题之后的 `---` 不再丢行：作为独立分隔线保留（序列化器按块分隔补空行）', () => {
		// 旧行为整行丢弃；现在保留，且空行由序列化器的块分隔规则补出（原文无空行）
		expect(roundTrip('- a\n---').out1).toBe('- a\n\n---');
		expect(roundTrip('# H\n---').out1).toBe('# H\n\n---');
	});
});
