...
| Add a link | Select a node → toolbar/menu **Add link** (pick a vault note or paste a URL). A wikilink turns an empty / pure-wikilink node into a pure wikilink (text = display name); a node with descriptive text gets a linked child node; URLs stay icon-only |
| Add an image | Select a node → **Add image** (from vault, clipboard, or a file) |
| Attach by drag & drop | Drag a vault note or attachment from the file explorer onto a selected node — it becomes the node's wikilink (empty / pure-wikilink node: text becomes the display name; node with descriptive text: a linked child node is created); dragging an image sets the node image. With nothing selected, a linked node is created under the root (images need a selected node) |
