...
// 节点数据快照：值级深拷贝（不进 JSON）——MindMapNode.update 每轮每节点生成，
// 并经 layouts/Base 的脏值比较消费（2026-09-25 起取代 JSON.stringify 快照：
// 「1 parse + 3 stringify/节点/轮」是空 render 26ms 的主体，见 vendor/BUILD.md 补丁清单）。
export const cloneNodeData = data => {
  if (data === null || typeof data !== 'object') return data
  if (Array.isArray(data)) {
    const res = new Array(data.length)
    for (let i = 0; i < data.length; i++) {
      res[i] = cloneNodeData(data[i])
    }
    return res
  }
  const res = {}
  ...
  for (let i = 0; i < keys.length; i++) {
    const key = keys[i]
    res[key] = cloneNodeData(data[key])
  }
  return res
}

// 通用值级深比较（NaN 视为相等，对齐全 JSON 语义——两侧同值 NaN 不视为变化，
// 避免「NaN 永久判脏 ⇒ 每轮重算」的性能退化）
const isSameDeepValue = (a, b) => {
  if (a === b) return true
  ...
  if (aIsArray) {
    if (a.length !== b.length) return false
    for (let i = 0; i < a.length; i++) {
      if (!isSameDeepValue(a[i], b[i])) return false
    }
    return true
  }
  const keysA = Object.keys(a)
  ...
  for (let i = 0; i < keysA.length; i++) {
    ...
    if (!isSameDeepValue(a[key], b[key])) return false
  }
  return true
}

// 节点数据的「是否变化」判定：忽略 isActive / expand（与老实现临时覆写该两字段
// 再比较的口径一致——激活/展开状态变化不触发节点尺寸与布局重算）。
const SNAPSHOT_IGNORED_KEYS = ['isActive', 'expand']

export const isNodeDataChanged = (prev, cur) => {
  // 快照异常（非普通对象）按「已变化」处理（安全方向：触发重建）
  if (prev === null || typeof prev !== 'object' || Array.isArray(prev))
    return true
  if (cur === null || typeof cur !== 'object' || Array.isArray(cur)) return true
  const prevKeys = Object.keys(prev)
  for (let i = 0; i < prevKeys.length; i++) {
    const key = prevKeys[i]
    if (SNAPSHOT_IGNORED_KEYS.indexOf(key) !== -1) continue
    if (!Object.prototype.hasOwnProperty.call(cur, key)) return true
    if (!isSameDeepValue(prev[key], cur[key])) return true
  }
