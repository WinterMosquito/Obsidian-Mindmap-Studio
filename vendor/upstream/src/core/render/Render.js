...
class Render {
  ...
  _render() {
    ...
    this.layout.doLayout(root => {
      ...
      // 打开/重建大图时消除 1s 级 UI 冻结。async 通道本身是 fix.3 既有实现
      // （性能模式视口变化路径 onViewDataChange 一直以 async=true 运行，
      // 见本文件 bindEvent），本补丁只是把 _render 主路径也接上；未注入该
      // opt 键（undefined）时为同步渲染，行为与上游一致。
      this.root.render(
        () => {
          this.isRendering = false
