...
// 测量自定义节点内容元素的宽高
// —— 自有补丁（2026-09-25 性能轮，见 vendor/BUILD.md「补丁清单」）——
// 自绘内容尺寸的**内容寻址缓存**：原测量 = 清空离屏容器 + appendChild +
// getBoundingClientRect（每节点一次强制 reflow），5000 节点打开实测测量段
// ≈ 0.15ms/节点（K72）。自绘内容全部内联样式（K53 ⑤，导出保真的要求）⇒
// outerHTML 即完整尺寸 key；缓存为**模块级**（跨引擎实例存活）⇒ 同一文件
// 重复打开（生产高频：切回/重开）命中率 ≈ 100%，命中时连 cloneNode 一并省。
// 安全性：key 覆盖全部尺寸决定因素（font-size / font-family / line-height /
// width / padding / white-space 皆内联在内容元素上）；测量上下文与原实现
// 完全一致（同一离屏容器）；**字体未就绪（web font 加载中）的度量可能偏
// fallback，此时不写缓存**，防污染。
// 容量须 ≥ 文档工作集：**顺序重扫 + 淘汰**对「容量不足」极其敏感——实测
// 5000 节点文档在 4096 容量下第二次打开仅省 261ms（命中率被顺序淘汰击穿），
// 放大到 ≥工作集后省 518ms。16384 条 ≈ 覆盖 1.6 万节点文档；条目平均约
// 0.3–1KB（outerHTML key + 两个数字），最坏内存约 16MB（多数在 2–8MB）。
const MEASURE_CACHE_MAX = 16384
const measureContentSizeCache = new Map()
function measureCacheGet(key) {
  const hit = measureContentSizeCache.get(key)
  if (hit) {
    // LRU 提升：删后重插（Map 迭代顺序 = 插入顺序）
    measureContentSizeCache.delete(key)
    measureContentSizeCache.set(key, hit)
  }
  return hit
}
function measureCachePut(key, size) {
  if (measureContentSizeCache.has(key)) {
    measureContentSizeCache.delete(key)
  } else if (measureContentSizeCache.size >= MEASURE_CACHE_MAX) {
    // 满：淘汰最旧一条（Map 首键）——上游 Lru 类满时拒绝新条目（#467），
    // 缓存需要真淘汰语义，故此处自建
    const oldest = measureContentSizeCache.keys().next().value
    if (oldest !== undefined) measureContentSizeCache.delete(oldest)
  }
  measureContentSizeCache.set(key, size)
}

function measureCustomNodeContentSize(content) {
  // 自有补丁：内容寻址缓存（命中直接返回，见上方说明）
  const cacheKey = content.outerHTML
  const cached = measureCacheGet(cacheKey)
  // 诊断（临时）：正式路径的命中/未命中计数，供 --bench-open 读取
  try {
    const doc = content.ownerDocument
    const view = doc && (doc.defaultView || doc.parentWindow)
    if (view) {
      const s = view.__MEASURE_STATS__ || (view.__MEASURE_STATS__ = { hit: 0, miss: 0 })
      if (cached) {
        s.hit++
      } else {
        s.miss++
  ...
  // 克隆移入本函数（调用方原传 cloneNode 产物）：命中时省下克隆本身
  ...
  this.mindMap.commonCaches.measureCustomNodeContentSizeEl.appendChild(
    contentClone
  )
  let rect =
...
// 命名导出（补丁 5 的预测量模块与正式测量路径共用同一缓存读写入口）
export { measureCacheGet, measureCachePut }
