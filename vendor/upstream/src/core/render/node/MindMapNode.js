...
class MindMapNode {
  ...
  createNodeData(recreateTypes) {
    ...
    if (
      ...
      // 自有补丁（补丁 5，A2）：优先复用「首帧前预测量」阶段构建的内容元素
      // （同函数、同数据、同样式来源，逐字节一致；见 src/core/render/preMeasure.js）
      // ——命中即省一遍内容构建；未命中/已消费走原构建，无副作用。
      const preMap = this.mindMap.__preMeasuredContentMap
      let reused = null
      if (preMap) {
        const uid = this.getData('uid')
        if (typeof uid === 'string' && uid) {
          reused = preMap.get(uid) || null
          if (reused) preMap.delete(uid)
        }
      }
      this._customNodeContent = reused || customCreateNodeContent(this)
    }
  ...
  update(forceRender) {
    ...
    // 保存一份当前节点数据快照（值级深拷贝；2026-09-25 起不再 JSON.stringify——
    // 每节点每轮的序列化是空 render 成本主体，见 vendor/BUILD.md 补丁清单）
    this.nodeDataSnapshot = readonly ? '' : cloneNodeData(this.getData())
  ...
  render(callback = () => {}, forceRender = false, async = false) {
    ...
    if (
      ...
      if (!this.group) {
        ...
        // O(1) 直接父判断（2026-09-25 自有补丁）：原 nodeDraw.has(group) 走
        // svg.js `has → index` = `[].slice.call(childNodes).indexOf(node)`——
        // 每节点每轮把 500 个兄弟节点全量拷进新数组再线性查找，500 节点图
        // 实测约 27ms/轮（--perf-ops=rendertick）。语义等价：has 只判「直接子」，
    ...
    if (
      ...
      this.children.forEach(item => {
        ...
        if (async) {
          // 销毁断链守卫（2026-09-25 自有补丁）：分片链在引擎 destroy
          // （el 置 null，见 index.js destroy）后仍留在 setTimeout 队列里，
          // 缺守卫时会继续在游离 DOM 上白跑（5000 节点图实测可多跑数百毫秒
          // 无效渲染）。el 已置空即停止派发——实例已死，回调链无消费者，
          // isRendering 等状态随实例一并作废。
          setTimeout(() => {
            if (!this.mindMap.el) return
            renderChild()
          }, 0)
        } else {
          renderChild()
  ...
  checkEnableDragModifyNodeWidth() {
    const { enableDragModifyNodeWidth } = this.mindMap.opt
    // 自有补丁（补丁 5，A2 配套）：以**实例事实**判定「该节点是否真的有自绘
    // 内容」（isUseCustomNodeContent() = !!this._customNodeContent），而非 opt
    // 开关「插件注册了自绘钩子」——A2 元素复用后内容可不经构建回调创建，opt
    // 开关无法区分「有内容 / 无内容」的节点（会给出死手柄）。实例口径与拖宽
    // 处理内部的三重检查（nodeModifyWidth）一致。
    return (
      enableDragModifyNodeWidth &&
      (this.mindMap.richText || this.isUseCustomNodeContent())
    )
  }
