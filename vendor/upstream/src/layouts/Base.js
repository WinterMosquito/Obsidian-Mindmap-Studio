...
import { createUid, cloneNodeData, isNodeDataChanged } from '../utils/index'
...
class Base {
  ...
  checkIsNodeDataChange(lastData, curData) {
    if (lastData) {
      // 值级深比较：对比忽略激活状态和展开收起状态（口径与原实现一致）。
      // 2026-09-25 起取代 JSON 序列化比较——原先每节点每轮需 1 parse + 3 stringify
      // （快照生成 + 此处两端），是空 render 26ms 的主体，见 vendor/BUILD.md 补丁清单。
      return isNodeDataChanged(lastData, curData)
    } else {
      // 只在都有数据时才进行对比
      return false
    }
  ...
  createNode(data, parent, isRoot, layerIndex, index, ancestors) {
    ...
    if (data && data._node && !this.renderer.reRender) {
      ...
      // 保存该节点上一次的数据（值级快照，同 MindMapNode.update 口径）
      const lastData = cloneNodeData(newNode.getData())
      // 节点层级改变了
      const isLayerTypeChange = this.checkIsLayerTypeChange(
        newNode.layerIndex,
        layerIndex
  ...
  updateChildren(children, prop, offset) {
    // 自有补丁 8（2026-09-25）：字段快路径。`item[prop] += offset` 走 getter/
    // setter（`customTop || _top` 读 + 写 _top），单次含两次属性访问器调用；
    // 5000 节点逻辑结构图首帧实测本方法**平移 293 万节点次**（adjustTopValue
    // 的兄弟子树多级传播），访问器开销成为量级项。等价性：`_top`/`_left` 的
    // 唯一读点在 MindMapNode 的 getter 内部（全库无绕过读取），分量
    // `customTop/customLeft === undefined` 时 getter 恒返回字段本身——
    // 字段直写在「无该分量自定义位置」的节点上与 `item[prop] += offset`
    // 严格等价（有分量的节点走原路径，保持既有语义）。闭包化内层递归，
    // 省去每层重复解析 prop。
    const fastField =
      prop === 'top' ? '_top' : prop === 'left' ? '_left' : null
    const customField =
      prop === 'top' ? 'customTop' : prop === 'left' ? 'customLeft' : null
    const walk = list => {
      list.forEach(item => {
        if (fastField !== null && item[customField] === undefined) {
          item[fastField] += offset
        } else {
          item[prop] += offset
        }
  ...
  updateChildrenPro(children, props) {
    // 自有补丁 8：与 updateChildren 同口径的字段快路径（props 为
    // { prop: offset } 对象；分量无自定义位置的节点字段直写）。
    const keys = Object.keys(props)
    const fields = keys.map(key =>
      key === 'top' ? '_top' : key === 'left' ? '_left' : null
    )
    const customFields = keys.map(key =>
      key === 'top' ? 'customTop' : key === 'left' ? 'customLeft' : key
    )
    const walk = list => {
      list.forEach(item => {
        keys.forEach((prop, i) => {
          if (fields[i] !== null && item[customFields[i]] === undefined) {
            item[fields[i]] += props[prop]
          } else {
