...
**产物字节级身份**：`vendor/simple-mind-map.cjs` 的 sha256 =
`a97b0caab190f14e9f814ecbd932dac186780537949afe65f95aa2c4ae1b8382`（406,580 B），
由 `tests/vendor-contract.test.ts` 断言。**重新打包后必须同步更新此处与该测试里的常量**——
它是唯一能发现「产物被手工编辑、或被换成来源不同的同名文件」的护栏：其余契约断言
（导出类 / 原型方法 / 命令名 / 事件名令牌）都只钉在 **API 面**上，对字节改动无感。
（缺口来源：本文件第 22 行要求「不可手工编辑」，但此前没有任何机械保障。）
