...
- 上游 `simple-mind-map` 以压缩产物分发，未携带逐文件版权横幅；`BUILD.md` 的重建命令
  已追加 `--legal-comments=inline`，**自 2026-09-25 重打包起**，各依赖的许可注释已
  **内联进产物**（`vendor/simple-mind-map.cjs`，406,583 B，sha256 见 `BUILD.md`），
  满足 MIT/BSD 的「保留版权与许可声明」要求。
- 本文件继续承担归属与许可声明作用（双重保障）：产物内联注释与本文档互为佐证；
  2026-09-25 之前的历史产物（406,580 B）则完全依赖本文件声明。
